...
LOG_CSV = os.path.join(BASE_DIR, "WER_Full.csv")

AUDIO_SUFFIXES = ["noisy", "denoised_nr", "noisy_DeepFilterNet2", "denoised_vad"]

MODELS = ["parakeet","fastconformer", "medium", "large"]
    ...
    for folder in tqdm(all_folders, desc=f"Calculating WER", total=len(all_folders)):   
        ...
        for suffix in AUDIO_SUFFIXES:
            for model_name in MODELS:
                hyp_file = os.path.join(folder_path, f"{folder}_{suffix}_{model_name}.txt")
